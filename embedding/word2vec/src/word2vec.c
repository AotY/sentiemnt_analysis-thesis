...
int searchVocab(char *word) {
    unsigned int hash = getStrHash(word);
    while (1) {
...
// Reads a word(or pinyin) and returns its index in the vocabulary
int readStrIndex(FILE *fin) {
    char str[MAX_STRING];
    readStr(str, fin);
    if (feof(fin))
        return -1;
    return searchVocab(str);
}
